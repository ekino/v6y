...
const createAuditHelp = async (auditHelp) => {
    try {
        AppLogger.info(`[AuditHelpProvider - createAuditHelp] auditHelp title:  ${auditHelp?.title}`);
        if (!auditHelp?.title?.length) {
            return null;
        }
        const createdAuditHelp = await AuditHelpModelType.create(auditHelp);
        AppLogger.info(`[AuditHelpProvider - createAuditHelp] createdAuditHelp: ${createdAuditHelp?._id}`);
...
const editAuditHelp = async (auditHelp) => {
    try {
        ...
        AppLogger.info(`[AuditHelpProvider - editAuditHelp] editedAuditHelp: ${editedAuditHelp?.[0]}`);
        return {
            _id: auditHelp?._id,
        };
    }
...
const getAuditHelpListByPageAndParams = async ({ start, limit, sort }) => {
    try {
        ...
        AppLogger.info(`[AuditHelpProvider - getAuditHelpListByPageAndParams] auditHelpList: ${auditHelpList?.length}`);
...
const getAuditHelpDetailsByParams = async ({ _id, category }) => {
    try {
        ...
        AppLogger.info(`[AuditHelpProvider - getAuditHelpDetailsByParams] auditHelpDetails _id: ${auditHelpDetails?._id}`);
