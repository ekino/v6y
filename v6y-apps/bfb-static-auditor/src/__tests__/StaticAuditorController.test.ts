...
beforeAll(async () => {
    ...
    runAnalysis = vi.spyOn(StaticAnalysisQueueService.prototype, 'runStaticAnalysis');
    endpoint = `${ServerConfig.currentConfig?.staticAuditorApiPath}/start-static-auditor.json`;
    app = await createApp();
});
